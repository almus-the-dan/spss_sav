...
const COMPRESSION_BYTECODE: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/tests/fixtures/compression_bytecode.sav"
);

const COMPRESSION_ZLIB: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/tests/fixtures/compression_zlib.sav"
);

const ZSAV_BLOCKS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/tests/fixtures/zsav_blocks.sav"
);
...
// ---------------------------------------------------------------------
// Data records — Uncompressed and bytecode
//
...
// every one of them. The uncompressed and bytecode files are read here;
// the ZSAV fixture joins this suite as 6(c) lands.
// ---------------------------------------------------------------------
...
/// The bytecode file holds the same three rows as the uncompressed one.
/// Every command byte is represented: inline codes for the small
/// integers, `253` for the values outside that range and for the short
/// strings, `254` for the all-spaces cell, `255` for the sysmis column.
#[test]
fn bytecode_rows_decode_to_their_written_values() {
    let mut reader = record_reader(COMPRESSION_BYTECODE);
    assert_eq!(reader.header().compression(), CompressionKind::Bytecode);
    assert_eq!(reader.case_count(), Some(3));
...
/// The two files are the same data written two ways, so they must decode
/// to the same cells. Compression is a storage detail and nothing above
/// the row source is allowed to see it.
#[test]
fn bytecode_and_uncompressed_files_decode_to_the_same_rows() {
    let mut bytecode = record_reader(COMPRESSION_BYTECODE);
    let mut uncompressed = record_reader(COMPRESSION_NONE);
    let mut rows = 0;
    while let Some(record) = uncompressed.read_record().expect("read uncompressed") {
        let expected = describe_row(&record);
        let record = bytecode
            .read_record()
            .expect("read bytecode")
            .expect("the same row");
        assert_eq!(describe_row(&record), expected, "row {rows}");
        rows += 1;
    }
    assert_eq!(rows, 3);
    assert!(bytecode.read_record().expect("read past end").is_none());
}

/// ZSAV is the same command stream inside a block container, so the
/// third file must decode to the same three rows as the other two.
#[test]
fn zlib_rows_decode_to_their_written_values() {
    let mut reader = record_reader(COMPRESSION_ZLIB);
    assert_eq!(reader.header().compression(), CompressionKind::Zlib);
    assert_eq!(reader.case_count(), Some(3));
...
/// The whole point of the three fixtures: one dataset, three encodings,
/// and nothing above the row source may be able to tell them apart.
#[test]
fn every_compression_scheme_decodes_to_the_same_rows() {
    let mut readers = [
        record_reader(COMPRESSION_NONE),
        record_reader(COMPRESSION_BYTECODE),
        record_reader(COMPRESSION_ZLIB),
    ];
    for expected in COMPRESSION_ROWS {
    ...
    for (index, reader) in readers.iter_mut().enumerate() {
        assert!(
            reader.read_record().expect("read past end").is_none(),
            "reader {index}",
        );
    }
}

/// The multi-block fixture. Every row carries its own number in `id`,
/// so a decoder that loses its place across the refill produces the
/// wrong one and this sees it — the block boundary falls inside row
/// 1856, one command into a group whose payloads are in the next block.
#[test]
fn a_multi_block_zsav_file_reads_across_the_block_boundary() {
    let mut reader = record_reader(ZSAV_BLOCKS);
    assert_eq!(reader.header().compression(), CompressionKind::Zlib);
    assert_eq!(reader.case_count(), Some(2000));

    let mut rows = 0_u32;
    while let Some(record) = reader.read_lazy_record().expect("read row") {
        rows += 1;
        let id = record.value(0).expect("the id cell");
        assert_eq!(describe_value(&id), rows.to_string(), "row {rows}");
    }
    assert_eq!(rows, 2000);
    assert!(reader.warnings().is_empty(), "{:?}", reader.warnings());
}

/// Pins why both ZSAV fixtures exist. `compression_zlib.sav` holds one
/// block and so never refills; `zsav_blocks.sav` is here to cross a
/// boundary, and a regeneration that shrank it below 0x3ff000 bytes of
/// command stream would silently stop testing that.
#[test]
fn the_zsav_fixtures_cover_both_the_single_and_multi_block_cases() {
    assert_eq!(zsav_block_count(COMPRESSION_ZLIB), 1);
    assert_eq!(zsav_block_count(ZSAV_BLOCKS), 2);
}

/// Reads the ZSAV trailer's block table straight off the end of the
/// file, so a test can assert on framing the reader deliberately does
/// not expose.
///
/// The trailer occupies the file's last `24 + 24 * n_blocks` bytes and
/// states `n_blocks` itself, so the count is recovered by trying
/// candidates until one is self-consistent.
fn zsav_block_count(path: &str) -> i32 {
    let bytes = std::fs::read(path).expect("read fixture");
    for blocks in 1..64_i32 {
        let trailer_len = 24 + 24 * usize::try_from(blocks).expect("a small count");
        let Some(start) = bytes.len().checked_sub(trailer_len) else {
            break;
        };
    ...
    panic!("no self-consistent ZSAV trailer in {path}");
}

/// A second, independently generated bytecode file — 43 data units to a
/// row, so its groups straddle on a different stride than the
/// compression fixture's 45. Values checked against `pspp-convert`'s own
/// CSV of the same file — which prints a declared-missing cell as its
/// ordinary value, so row 1's `longstr` reads as `alpha` there while
/// `MISSING VALUES longstr ('alpha')` makes it missing here.
#[test]
fn comprehensive_rows_decode_through_the_bytecode_decoder() {
    let mut reader = finalize_comprehensive();
    let mut rows = Vec::new();
    while let Some(record) = reader.read_record().expect("read record") {
        rows.push(describe_row(&record));
    }
    assert_eq!(
        rows,
        [
            ["1", "1", "0", "1", "missing(\"alpha\")", "\"aa\""],
            ["2", "0", "1", "1", "\"beta\"", "\"bb\""],
        ],
    );
}

/// What makes this fixture worth having: 45 data units to a row against
/// eight-command groups, so rows 2 and 3 each begin partway through a
/// group. A decoder that started every row on a fresh group would pass
/// on an aligned file and fail here — which is why the alignment is
/// asserted rather than assumed.
#[test]
fn the_bytecode_fixture_makes_command_groups_straddle_rows() {
    let units_per_row = COMPRESSION_ROW_LEN / 8;
    assert_eq!(units_per_row, 45);
    assert_ne!(
        units_per_row % 8,
        0,
        "an aligned row would not exercise the carry",
    );
}
