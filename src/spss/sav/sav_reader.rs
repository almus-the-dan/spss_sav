...
//!
//! First phase of the reader typestate chain. Created via
//! [`SavReaderBuilder::from_path`](crate::spss::sav::sav_reader_builder::SavReaderBuilder::from_path)
...
/// Created via
/// [`SavReaderBuilder::from_path`](crate::spss::sav::sav_reader_builder::SavReaderBuilder::from_path)
/// (or [`from_file`](crate::spss::sav::sav_reader_builder::SavReaderBuilder::from_file)
/// /
/// [`from_reader`](crate::spss::sav::sav_reader_builder::SavReaderBuilder::from_reader)).
/// Nothing has been read yet: opening a file cannot tell you anything
/// about it, so every accessor worth having lives on what the two
/// terminal methods return.
///
/// Call [`into_record_reader`](Self::into_record_reader) to go straight
/// to the values, or [`into_dictionary_reader`](Self::into_dictionary_reader)
/// to walk the dictionary first. Both consume the reader, and both read
/// the 176-byte header and the whole dictionary section on the way —
/// unavoidably, since a SAV file declares its text encoding at the *end*
/// of the dictionary, after every string that encoding governs. They
/// differ only in how much of what they pass is kept.
#[derive(Debug)]
...
impl<R> SavReader<R> {
    /// Wraps an opened reader, forwarding the options set on the
    /// upstream
    ...
    /// [`SavReaderBuilder::encoding_strategy`](crate::spss::sav::sav_reader_builder::SavReaderBuilder::encoding_strategy).
    #[must_use]
    #[inline]
